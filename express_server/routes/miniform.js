...
router.post('/updateForm', (req, res) =>{
    ...
    Miniform.findByIdAndUpdate(req.body.miniformId, {$set: req.body}, {new: true},  (err, doc) => {
...
router.post('/updateQuestion', (req, res) =>{
    ...
    Question.findByIdAndUpdate(req.body.questionId, {$set: req.body}, {new: true},  (err, doc) => {
        if(err) return res.json({ success: false, err: err })
        return res.status(200).json({ success: true, doc: doc })
    });    
})
...
router.post('/updateChoice', (req, res) =>{
    ...
    Choice.findByIdAndUpdate(req.body.choiceId, {$set: req.body}, {new: true},  (err, doc) => {
        if(err) return res.json({ success: false, err: err })
        return res.status(200).json({ success: true, doc: doc })
    });    
})
...
router.post('/updateMiniformAnswer', (req, res) =>{
    ...
    Answer.findOneAndUpdate(req.body.filters, {$set: req.body.setValues}, {new : true}, (err, answer)  => {
        if(err) return res.json({ success: false, err: err })
        return res.status(200).json({ success: true, answer: answer})
    });
