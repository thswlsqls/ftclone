...
const [isCommented, setisCommented] = useState(false)
    ...
    const onCommentLike = () =>{
        ...
        console.log("댓글에 좋아요를 등록합니다. 좋아요를 등록하는 유저의 아이디는" +user.userData._id + "이 댓글의 아이디는" + props.comment._id+"이 댓글이 속한 게시글의 아이디는"+props.postId)
        var submitData = {
            userId: user.userData._id,
            postId: props.postId,
            commentId: props.comment._id
        ...
        .then(response => {
            if(response.data.success){
                alert('유저가 이 댓글에 좋아요를 등록했습니까?  '+response.data.isLike);
    ...
    const onReplyOpen = (comment) => {
        ...
        Axios.post("/api/comments/ReplyComment", submitData) 
            .then(response => {
                if(response.data.success) {
                    alert("the reply comment saved successfuly");
        ...
        <div>
            <li style={{listStyleType:'none', textAlign: 'match-parent'}}>
                                    ...
                                    <div className="div_postUnit_ViewAndAddComments_Comment_content__userId">
                                        <span>
                                            {  
                                                ...
                                                props.comment.writer.user_name
                                            }
                                        </span>
                                    </div>
                ...
                <div className="div_postUnit_ViewAndAddComments_moreComments">
                    {IsReplyOpen&& <ReplyComment ThisPostCommentList={props.ThisPostCommentList} postId={props.postId} parentComment={props.comment} />}
                </div>
            </li>
        </div>
    )
